...
1. User is to get a Apple Credential through signing in to their Apple ID through the AppleCredentialProvider.
...
2. After getting Apple Credential, we will check if the user has already signed up with the corresponding Apple Credential before. A user who already has an account with Portkey should have one or more Guardians returned from the callback.
