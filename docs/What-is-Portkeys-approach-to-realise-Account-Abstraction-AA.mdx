...
Portkey, the first <u>Account Abstraction (AA)</u> wallet in the <u>aelf</u> ecosystem, employs aelf's Virtual Address (VA) design to implement AA. Instead of relying on private keys, user accounts, known as virtual addresses in Portkey, are managed by manager addresses stored on users' local devices. This innovative approach enables advanced features, such as social recovery, payment delegation, batched transactions, and more.
...
To log in to an account on a new device, users must successfully complete <u>social recovery</u>. This process requires obtaining sufficient approvals from guardians. Only when the approval requirements are met can the user log in, which generates a new manager address on the target device, allowing control of their on-chain account.
